...
function repeatString(str, times) {
  return str.repeat(times < 0 ? 0 : times);
}
...
function removeFirstOccurrences(str, value) {
  ...
  const hasSubstr = index !== -1;

  if (!hasSubstr) return str;
...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((total, cur) => total + cur.charCodeAt(0), 0);
}
...
function orderAlphabetically(str) {
  return str.split('').sort(new Intl.Collator('en').compare).join('');
}
...
function isPalindrome(str) {
  const normalized = str.replaceAll(/\W/gi, '').trim().toLowerCase();

  for (let i = 0; i < Math.floor(normalized.length / 2); i += 1) {
